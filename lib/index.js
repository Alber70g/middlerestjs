...
function Rest() {
  this.middleware = [];
  this.defineDbMiddleware = (rest) => {
    rest.on({ method: 'GET' }, (req, res, next) => {
      console.log('executing defineDbMiddleware with {method: \'GET\'}')
      if (this.pathSegments.length % 2) {
        ...
        this.select(this.pathSegments[0])
          ...
          .catch((error) => {
            console.log('error defineDbMiddleware array', error);
            return next(error);
          });
      } else {
        ...
        this.select(this.pathSegments[0])
          ...
          .catch((error) => {
            console.log('error in defineDbMiddleware single', error);
            return next(error);
          });
      }
    });
...
Rest.prototype = {
  ...
  listen(port, url = '/*') {
    ...
    // Add last 'regular' middleware: dbMiddleware
    this.defineDbMiddleware(this);
